        ...
        {(posts as any[]).length > 0 && (
          <div>
            ...
            <ul className="space-y-1.5">
              {(posts as any[]).map((p:any)=><li key={p.slug}><Link href={`/${p.slug}`} className="text-sm text-gray-700 hover:text-[#e63329] hover:underline flex items-center gap-1"><span className="text-gray-400">›</span>{p.title}</Link></li>)}
            </ul>
          </div>
        )}
      </div>

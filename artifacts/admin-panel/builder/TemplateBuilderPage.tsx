...
/* ── Top Bar ── */
function TopBar({ title, saving, lastSaved, saveError, viewport, themeColor, previewCategory, onSave, onBack, onUndo, onRedo, onViewport, onPreview, onOpenCode, onOpenHistory, onSaveBlock, onChangeTheme, onSelectSampleCategory }: {
  title: string; saving: boolean; lastSaved: string; saveError: string; viewport: string; themeColor: string; previewCategory: string;
  ...
  return (
    <header style={{ height: 56, flexShrink: 0, background: '#0D1F3C', borderBottom: '1px solid rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', gap: 6, padding: '0 16px', fontFamily: 'Outfit, Inter, sans-serif', zIndex: 200, boxShadow: '0 4px 20px rgba(0,0,0,0.3)' }}>
      <button onClick={onBack} style={{ ...topBtn(), background: 'rgba(255,255,255,0.08)', color: '#FFF' }} title="Back to Templates">
        {Ico.back}
      </button>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginLeft: 6 }}>
        ...
        <div>
          ...
          <span style={{ color: '#FFB800', fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em' }}>Template Visual Editor</span>
        </div>
      </div>

      <div style={{ flex: 1 }} />

      {/* Sample Data Live Preview Dropdown */}
      ...
      <div style={{ display: 'flex', borderBottom: '1px solid #1E293B', flexShrink: 0, background: '#0B0F19', padding: '8px 12px', gap: 6 }}>
        {(['blocks', 'layers'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)} style={{
            ...
            display: 'flex', alignItems: 'center', justifyCenter: 'center', gap: 6, transition: 'all .15s'
          }}>
...
export default function TemplateBuilderPage() {
  ...
  const title = TEMPLATE_LABELS[type] ?? type;
  ...
  const triggerSave = useCallback(async () => {
    ...
    try {
      ...
      setRevisions(prev => [{ id: `rev_${Date.now()}`, time, html, css }, ...prev.slice(0, 9)]);
  ...
  /* ── Load template ── */
  useEffect(() => {
    fetch(`/api/admin/templates/${type}`, { credentials: 'include' })
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden', background: '#0B0F19', fontFamily: 'Inter, sans-serif' }}>
      <style>{GJS_CSS}</style>
      <TopBar title={title} saving={saving} lastSaved={lastSaved} saveError={saveError} viewport={viewport} themeColor={themeColor} previewCategory={previewCategory}
        onSave={triggerSave} onBack={() => nav('/templates')}

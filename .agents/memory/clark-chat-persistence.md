...
**How to apply:** Await the conversation save before provider work and save the completed assistant/admin response afterward. If an AI provider returns a model-not-found error, verify its current production model catalog before changing keys or asking the user for credentials.
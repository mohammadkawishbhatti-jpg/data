...
function DescriptionContent({ text }: { text: string }) {
  ...
  if (hasHtml) {
    return (
      <div
        className="prose prose-sm max-w-none prose-headings:text-[#1a2f5a] prose-headings:font-bold prose-a:text-[#e63329] prose-li:text-gray-600 prose-h2:text-xl prose-h2:mt-6 prose-h2:mb-2 prose-h3:text-base prose-h3:mt-4 prose-h3:mb-1"
  ...
  return (
    <div className="space-y-4 text-gray-600 leading-relaxed">
      {first && <p className="font-semibold text-[#1a2f5a] text-lg">{first}</p>}
      {rest.map((p, i) => <p key={i} className="text-sm md:text-base">{p.replace(/\n/g, " ")}</p>)}
    </div>
  );
}
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center flex-wrap text-xs text-white/50 gap-1 mb-3">
                ...
                {product.categoryName && (
                  <>
                    <ChevronRight className="h-3 w-3" />
                    <Link href={`/category/${product.categorySlug || product.categoryId}`} className="hover:text-white/90 transition-colors">{product.categoryName}</Link>
                  </>
                )}
                <ChevronRight className="h-3 w-3" />

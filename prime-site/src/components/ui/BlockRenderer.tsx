    ...
    <div className="py-20 px-4 text-center" style={{ background: d.bgColor || "#1a2f5a", color: d.textColor || "#ffffff" }}>
      <div className="max-w-3xl mx-auto">
        ...
        {d.buttonText && d.buttonLink && (
          <Link href={d.buttonLink} className="inline-block bg-[#e63329] text-white px-8 py-3 rounded-lg font-bold hover:bg-red-700 transition-colors">{d.buttonText}</Link>
        )}
      </div>
    </div>
  );
...
function TextBlock({ d }: { d: any }) {
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      <div className={`prose max-w-none ${d.align === "center" ? "text-center" : d.align === "right" ? "text-right" : ""}`} dangerouslySetInnerHTML={{ __html: d.content || "" }} />
    </div>
  );
}
    ...
    <div className="max-w-6xl mx-auto px-4 py-14">
      <div className={`grid md:grid-cols-2 gap-10 items-center ${reversed ? "direction-rtl" : ""}`}>
        <div className={reversed ? "order-2" : ""}>
          ...
          {d.buttonText && d.buttonLink && (
            <Link href={d.buttonLink} className="inline-block bg-[#1a2f5a] text-white px-6 py-2.5 rounded-lg font-semibold hover:bg-[#1a2f5a]/90 transition-colors">{d.buttonText}</Link>
          )}
        </div>
  ...
  return (
    <div className="py-14 text-center px-4" style={{ background: d.bgColor || "#e63329" }}>
      <div className="max-w-3xl mx-auto text-white">
        ...
        {d.buttonText && d.buttonLink && (
          <Link href={d.buttonLink} className="inline-block bg-white text-[#e63329] px-8 py-3 rounded-lg font-bold hover:bg-gray-100 transition-colors">{d.buttonText}</Link>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-14">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
        <div className="prose max-w-none" dangerouslySetInnerHTML={{ __html: d.col1 || "" }} />
        <div className="prose max-w-none" dangerouslySetInnerHTML={{ __html: d.col2 || "" }} />
      </div>
    </div>
  );
}
...
function InlineBlock({ block }: { block: Block }) {
  ...
  switch (block.type) {
    ...
    case "heading": {
      const Tag = (d.tag || d.headingTag || "h2") as keyof JSX.IntrinsicElements;
    ...
    case "button": return d.link ? <Link href={d.link} className="inline-block bg-[#e63329] text-white px-6 py-2.5 rounded-lg font-bold hover:bg-red-700 transition-colors my-2">{d.text || "Learn More"}</Link> : null;
...
function renderPuckItem(item: any, zones: Record<string, any[]>): React.ReactNode {
  ...
  switch (item.type) {
        ...
        <div key={id} style={{ background: p.bgImage ? `url(${p.bgImage}) center/cover no-repeat` : p.background, color: p.textColor, paddingTop: p.paddingTop ?? 80, paddingBottom: p.paddingBottom ?? 80, minHeight: p.minHeight || undefined, display: "flex", alignItems: "center", position: "relative" }}>
          ...
          <div style={{ maxWidth: 900, margin: "0 auto", padding: "0 24px", textAlign: p.align ?? "center", position: "relative", width: "100%" }}>
            ...
            <div style={{ display: "flex", gap: 16, justifyContent: p.align === "center" ? "center" : "flex-start", flexWrap: "wrap" }}>
              {p.ctaText  && <a href={p.ctaUrl}  style={{ background: "#f97316", color: "#fff", padding: "14px 32px", borderRadius: 8, fontWeight: 700, textDecoration: "none", fontSize: 16 }}>{p.ctaText}</a>}
              {p.ctaText2 && <a href={p.ctaUrl2} style={{ background: "transparent", color: p.textColor, padding: "14px 32px", borderRadius: 8, fontWeight: 600, textDecoration: "none", fontSize: 16, border: `2px solid ${p.textColor}` }}>{p.ctaText2}</a>}
            </div>
          </div>
        </div>
      );
        ...
        <ul key={id} style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: p.gap ?? 12 }}>
          {(p.items ?? []).map((item: any, i: number) => (
            <li key={i} style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
              ...
              {item.link
                ? <a href={item.link} style={{ color: p.textColor ?? "#374151", fontSize: p.fontSize ?? 15, fontWeight: p.fontWeight ?? "400", textDecoration: "underline" }}>{item.text}</a>
    ...
    case "Icon": {
      ...
      return p.href ? <a key={id} href={p.href} style={{ textDecoration: "none" }}>{content}</a> : <div key={id}>{content}</div>;
    }
    ...
    case "Image": {
      ...
      return (
        <div key={id} style={{ display: "flex", flexDirection: "column", alignItems: p.align ?? "center" }}>
          {p.href ? <a href={p.href} target={p.target ?? "_self"}>{img}</a> : img}
    ...
    case "RawHtml":
      return <div key={id} className="prose prose-headings:text-[#1a2f5a] prose-headings:font-bold max-w-none" dangerouslySetInnerHTML={{ __html: p.html ?? "" }} />;
        ...
        <div key={id} style={{ height: p.height ?? 260, perspective: 1000 }} className="puck-flip">
          ...
          <div className="puck-flip-inner">
            ...
            <div className="puck-flip-face puck-flip-back" style={{ background: p.backBg ?? "#f97316", color: p.backColor ?? "#fff" }}>
              ...
              {p.backBtnText && <a href={p.backBtnUrl} style={{ display: "inline-block", background: "rgba(255,255,255,0.2)", color: p.backColor ?? "#fff", border: `2px solid ${p.backColor ?? "#fff"}`, padding: "8px 20px", borderRadius: 6, fontWeight: 700, textDecoration: "none", fontSize: 14 }}>{p.backBtnText}</a>}
            </div>
          </div>
        </div>
      );
      ...
      return (
        <div key={id} style={{ background: p.background ?? "#f97316", color: p.textColor ?? "#fff", padding: "60px 24px", textAlign: p.align ?? "center", borderRadius: p.borderRadius ?? 0 }}>
          ...
          <div style={{ display: "flex", gap: 14, justifyContent: p.align === "center" ? "center" : "flex-start", flexWrap: "wrap" }}>
            {p.buttonText  && <a href={p.buttonUrl}  style={{ display: "inline-block", background: p.buttonBg ?? "#fff", color: p.buttonColor ?? (p.background ?? "#f97316"), padding: "14px 36px", borderRadius: 8, fontWeight: 700, textDecoration: "none", fontSize: 16 }}>{p.buttonText}</a>}
            {p.buttonText2 && <a href={p.buttonUrl2} style={{ display: "inline-block", background: "transparent", color: p.textColor ?? "#fff", padding: "14px 36px", borderRadius: 8, fontWeight: 700, textDecoration: "none", fontSize: 16, border: `2px solid ${p.textColor ?? "#fff"}` }}>{p.buttonText2}</a>}
          </div>
        </div>
      );
    ...
    case "IconBox":
      return (
        <div key={id} style={{ background: p.background ?? "#f8fafc", color: p.textColor ?? "#1e293b", padding: p.padding ?? 32, borderRadius: p.borderRadius ?? 12, textAlign: p.align ?? "center", height: "100%", boxSizing: "border-box" as any, boxShadow: p.shadow ?? "none", border: `1px solid ${p.border ?? "#e2e8f0"}` }}>
          ...
          {p.link && <a href={p.link} style={{ color: "#2563eb", fontWeight: 600, fontSize: 14, textDecoration: "none" }}>{p.linkText || "Learn more →"}</a>}
        </div>
      );
              ...
              {(p.plans ?? []).map((plan: any, i: number) => {
                ...
                return (
                  <div key={i} style={{ background: featured ? (p.accentColor ?? "#1a2f5a") : "#fff", color: featured ? "#fff" : "#1e293b", borderRadius: 16, padding: "32px 24px", boxShadow: featured ? "0 12px 40px rgba(0,0,0,0.18)" : "0 2px 8px rgba(0,0,0,0.08)", border: featured ? "none" : "1px solid #e2e8f0", position: "relative", transform: featured ? "scale(1.04)" : "none" }}>
                    ...
                    <a href={plan.btnUrl} style={{ display: "block", textAlign: "center", background: featured ? "#fff" : (p.accentColor ?? "#1a2f5a"), color: featured ? (p.accentColor ?? "#1a2f5a") : "#fff", padding: "12px 0", borderRadius: 8, fontWeight: 700, textDecoration: "none", fontSize: 15 }}>{plan.btnText}</a>
                  </div>
                );
              })}
            </div>
...
export function BlockRenderer({ content }: { content: string }) {
  ...
  try { parsed = JSON.parse(content || "[]"); }
    ...
    return <div className="prose max-w-4xl mx-auto px-4 py-12" dangerouslySetInnerHTML={{ __html: content || "" }} />;
  }
...
@media (max-width: 767px) {
  ...
  .gjs-content [style*="font-size:5"] { font-size: clamp(28px, 8vw, 52px) !important; }
        ...
        ` }} />
        <div className="gjs-content" dangerouslySetInnerHTML={{ __html: parsed.gjs.html }} />
      </>
    );
  }
  ...
  else return <div className="prose max-w-4xl mx-auto px-4 py-12" dangerouslySetInnerHTML={{ __html: content || "" }} />;
          ...
          case "button":
            return d.link ? (
              <div key={block.id} className="max-w-4xl mx-auto px-4 py-4" style={{ textAlign: d.align || "center" }}>
                <Link href={d.link} className="inline-block bg-[#e63329] text-white px-8 py-3 rounded-lg font-bold hover:bg-red-700 transition-colors">
                  {d.text || "Learn More"}
                </Link>
              </div>
            ) : null;
            ...
            return (
              <div key={block.id} style={{
                ...
                {d.bgImage && d.bgOverlay > 0 && (
                  <div style={{ position: "absolute", inset: 0, background: `rgba(0,0,0,${d.bgOverlay / 100})` }} />
                )}
